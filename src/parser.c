...
CoulangDeclFunctionParam *parse_function_params_declaration__Parser(TokensIterator *ite) {
  ...
  while (current->kind != COULANG_TOKEN_KIND_RPAREN) {
	  CoulangToken *token_identifier = expect_token(COULANG_TOKEN_KIND_IDENTIFIER, ite);
	  const String *const name = &token_identifier->identifier;
	  enum CoulangDataType data_type = parse_data_type__Parser(ite);

	  CoulangToken *current = get_current_token__TokensIterator(ite);

	  CoulangDeclFunctionParam *param = init__CoulangDeclFunctionParam(name, data_type);
...
CoulangDecl *parse_function_declaration__Parser(TokensIterator *ite) {
  ...
  CoulangDeclFunctionParam *params = parse_function_params_declaration__Parser(ite);
